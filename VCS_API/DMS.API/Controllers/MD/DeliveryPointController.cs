...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class DeliveryPointController(IDeliveryPointService service) : ControllerBase
    {
        public readonly IDeliveryPointService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] DeliveryPointDto DeliveryPoint)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(DeliveryPoint);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] DeliveryPointDto DeliveryPoint)
        {
            var transferObject = new TransferObject();
            await _service.Update(DeliveryPoint);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
