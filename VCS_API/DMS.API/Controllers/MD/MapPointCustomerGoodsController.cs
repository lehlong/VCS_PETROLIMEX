...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class MapPointCustomerGoodsController(IMapPointCustomerGoodsService service) : ControllerBase
    {
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] MapPointCustomerGoodsDto MapPointCustomerGoods)
        {
            var transferObject = new TransferObject();
            await _service.Update(MapPointCustomerGoods);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
