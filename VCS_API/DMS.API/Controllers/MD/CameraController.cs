...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class CameraController(ICameraService service) : ControllerBase
    {
        public readonly ICameraService _service = service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] CameraDto Camera)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(Camera);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] CameraDto Camera)
        {
            var transferObject = new TransferObject();
            await _service.Update(Camera);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
