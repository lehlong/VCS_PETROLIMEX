...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class GoodsController(IGoodsService service) : ControllerBase
    {
        public readonly IGoodsService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] GoodsDto Goods)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(Goods);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] GoodsDto Goods)
        {
            var transferObject = new TransferObject();
            await _service.Update(Goods);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
