...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IPositionService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] PositionDto Position)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(Position);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] PositionDto Position)
        {
            var transferObject = new TransferObject();
            await _service.Update(Position);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
