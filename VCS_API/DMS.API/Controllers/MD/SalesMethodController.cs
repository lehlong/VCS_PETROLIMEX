...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class SalesMethodController(ISalesMethodService service) : ControllerBase
    {
        public readonly ISalesMethodService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] SalesMethodDto SalesMethod)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(SalesMethod);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] SalesMethodDto SalesMethod)
        {
            var transferObject = new TransferObject();
            await _service.Update(SalesMethod);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
