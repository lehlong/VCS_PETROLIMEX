...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class TypeOfGoodsController(ITypeOfGoodsService service) : ControllerBase
    {
        public readonly ITypeOfGoodsService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] TypeOfGoodsDto TypeOfGoods)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(TypeOfGoods);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] TypeOfGoodsDto TypeOfGoods)
        {
            var transferObject = new TransferObject();
            await _service.Update(TypeOfGoods);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
