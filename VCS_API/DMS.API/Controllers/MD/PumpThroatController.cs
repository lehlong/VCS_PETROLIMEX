...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class PumpThroatController(IPumpThroatService service) : ControllerBase
    {
        public readonly IPumpThroatService _service = service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] PumpThroatDto PumpThroat)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(PumpThroat);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] PumpThroatDto PumpThroat)
        {
            var transferObject = new TransferObject();
            await _service.Update(PumpThroat);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
