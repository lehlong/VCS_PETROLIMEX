...
namespace DMS.API.Controllers.IN
{
    ...
    [ApiController]
    public class VinhCuaLoController(IVinhCuaLoService service) : ControllerBase
    {
        public readonly IVinhCuaLoService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] VinhCuaLoDto vinhCuaLo)
        {
            var transferObject = new TransferObject();
            await _service.AddVinhCuaLo(vinhCuaLo);
            if (_service.Status)
            {
                //transferObject.Data = result;
